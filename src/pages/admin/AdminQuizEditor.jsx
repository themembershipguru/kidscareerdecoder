...
export function AdminQuizEditor() {
  ...
  const [labelingDifficulty, setLabelingDifficulty] = useState(false)
  ...
  async function labelDifficultyWithOpenAI() {
    if (
      !confirm(
        'Use OpenAI (gpt-4o-mini) to set difficulty 1–5 on every question in this quiz? Existing difficulty values will be overwritten. Requires OPENAI_API_KEY on the server.',
      )
    ) {
      return
    }
    setLabelingDifficulty(true)
    setError('')
    try {
      await api.post(`/admin/quizzes/${id}/label-difficulty-openai`)
      await load()
    } catch (err) {
      setError(getApiError(err))
    } finally {
      setLabelingDifficulty(false)
    }
  }
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <section className="mt-10">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <button
              type="button"
              disabled={labelingDifficulty || !(quiz.questions ?? []).length}
              onClick={() => void labelDifficultyWithOpenAI()}
              className="rounded-lg border border-violet-300 bg-violet-50 px-4 py-2 text-sm font-bold text-violet-900 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {labelingDifficulty
                ? 'Labeling with OpenAI…'
                : 'AI label difficulty (1–5)'}
            </button>
          </div>
          <p className="mt-1 text-xs text-slate-500">
            Writes <code className="rounded bg-slate-100 px-1">difficulty_level</code> on
            each question for adaptive quiz order. Quiz content stays in your database.
          </p>
                  ...
                  <div>
                    ...
                    {q.difficulty_level != null && q.difficulty_level !== '' && (
                      <p className="mt-0.5 text-xs text-violet-700">
                        Difficulty:{' '}
                        <span className="font-mono font-bold">{q.difficulty_level}</span>
                      </p>
                    )}
                  </div>
                  <button
